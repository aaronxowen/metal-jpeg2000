// Benchmark the OPTIMIZED (component-batched) GPU back-end vs the oracle.
// Mirrors proto/backend_bench.swift but uses one concatenated component buffer
// and the batched kernels (backend_opt.metal) -> ~10 dispatches, 3x occupancy.
//
// Build: swiftc -O proto/phase_a/backend_opt_bench.swift -o /tmp/backend_opt_bench -framework Metal -framework Foundation
// Run:   /tmp/backend_opt_bench proto/backend_corpus.bin proto/phase_a/backend_opt.metal
...
let kernelPath = args.count > 2 ? args[2] : "proto/phase_a/backend_opt.metal"
...
print("GPU: \(dev.name)  \(w)x\(h), \(nc) comps, \(numres) res, mct=\(mct)  [OPTIMIZED back-end]")
...
let psoH = try! dev.makeComputePipelineState(function: lib.makeFunction(name:"idwt97_h_b")!)
let psoV = try! dev.makeComputePipelineState(function: lib.makeFunction(name:"idwt97_v_b")!)
...
let wpool   = dev.makeBuffer(length: nc*stride*stride*4, options: .storageModePrivate)!
...
func run() -> Double {
    ...
    for lvl in 1..<numres {
        ...
        eh.setBuffer(dataBuf,offset:0,index:0); eh.setBuffer(boxBuf,offset:0,index:1); eh.setBuffer(wpool,offset:0,index:2)
        ...
        ev.setBuffer(dataBuf,offset:0,index:0); ev.setBuffer(boxBuf,offset:0,index:1); ev.setBuffer(wpool,offset:0,index:2)
...
print(String(format: "OPTIMIZED GPU back-end: %.3f ms/frame", best))
